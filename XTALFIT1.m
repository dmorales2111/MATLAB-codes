function [] = XTALFIT1(datax)
    ...
    global Minit1 Range1 dimtem theta ymin ymax redat 
    ...
    axis([min(Range1),max(Range1),ymin,ymax])
    ...
    title('Rotation about Xtal Axis')
...
function update_fitb(varargin)
    global Minit1 Range dimtem
    global theta ymin ymax redat Orienitx
    ...
    tempx=updatefig_1(Tenval,Orienitx,alf,bet,gam,offset,div1,div2); 
    subplot('position',[(fhleft-50)/fhwidth,(fhbottom-20)/fhheight,0.45,0.75])
    ...
    for i=2:dimtem(2)/2
        j=2*i-1;
        Thet=theta(j)+offset;
        tempx=updatefig_1(Tenval,Orienitx,alf,bet,gam,Thet,div1,div2);
...
function z = SPECTfit(Ten1,h,w) 
    ...
    for i=1:length(Range)
       F(i) = 0.3*h*exp(-(Range(i)-Ten1(3,3)).^2/(2*w^2)); % Gaussian broadened line   
    end
...
function [] = pb_load_xdata(varargin)
    ...
    global Range dimtem theta ymin ymax redat Orienitx
    ...
    Fitx = updatefig_1(Tenval,Orienitx,alf,bet,gam,ofst,h,w);
    subplot('position',[(fhleft-50)/fhwidth,(fhbottom-20)/fhheight,0.45,0.75])
    ...
    for i=2:dimtem(2)/2
        j=2*i-1;
        THET=theta(j)+ofst;
        tempx=updatefig_1(Tenval,Orienitx,alf,bet,gam,THET,h,w); %call the main fitting function for each theta
        plot(tempx(:,1),tempx(:,2)+theta(j),'r',redat(:,j),redat(:,j+1),'.k')
    end
    hold off
    xlabel('ppm') 
