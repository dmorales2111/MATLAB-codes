function [] = TENSOR_rot
    ...
    %and then adjusts the Euler angles in order to fit the data in the lab frame. 
    ...
    updatefig_rot(I0,prinax,a,b,c)   %initiates the 3-D plot
...
elseif Xor == 5
 ...
 Orien = Orient; %Reset the global (not sure how to handle this?)
    ...
    Mdat(1,1) = str2double(get(S.ed(5),'string'));
...
R = Rot*Rgamm*Rbeta*Ralfa*Orien; %rotates the "test-frame" w.r.t. to lab
Mdatout=R*Mdat*(R\I0);
...
%show the tensor (the components T"ij)
...
if get(S.ch,'Value') == get(S.ch,'Max') %note that for a checkbox the 'Max' state is 'when checked'
...
%Z_proj=[0 0 1]*(R*[ai bi ci]');
%set(S.txt(53),'str',num2str(Z_proj,4));    %set lab z-axis projection field (shift as measured in lab)

%update the figure (tensor and 3-D plot with cartesian axes lengths defined by px,ai,bi,ci)
updatefig_rot(R,px,ai,bi,ci)
end %vec_rot
...
R = Rg*Rb*Ra*Orien; %refine diagonalization 
Md = R*Mdat*(R\I0); 
...
%Z_pro=[0 0 1]*(R*[ai bi ci]');
%set(S.txt(53),'str',num2str(Z_pro,4));    %set lab z-axis projection field (shift as measured in lab)
...
%update the unit x-tal frame vectors (the vectors are the rows of R)
    set(S.txt(13),'str',num2str(R(1,1),4)); 
    set(S.txt(14),'str',num2str(R(2,1),4)); %blue xtal X"
    set(S.txt(15),'str',num2str(R(3,1),4));
...
%Z_pro=[0 0 1]*(R*[ai bi ci]');  %projection of tensor on Lab-z axis (along B0 - field direction)
%set(S.txt(53),'str',num2str(Z_pro,4));    %set lab z-axis projection field (shift as measured in lab)    
...
%**************************************************************************
function [] = updatefig_rot(Rp,prnx,aa,bb,cc) %Rp=Eigenvector matrix, aa=x-eigenvalue, bb=y-eigenvalue, cc=z-eigenvalue

%First define an axis upon which we will place our 3d plots
...
%     zlabel('lab-z');
    ...
    %subplot('Position',[(fhleft-35+3*fhwidth*0.06+40)/fhwidth 0.1*fhbottom/fhheight 0.45*fhwidth 0.8*fhheight])
    ...
    %Unit Vectors (...morphing towards Principal axes)
    Pnts = [tmp1';tmp2';tmp3']; %3 row x 3 column
    plot3(Ax1(:,1),Ax1(:,2),Ax1(:,3),'c','LineWidth',10) %x-tal X"
    plot3(Ax2(:,1),Ax2(:,2),Ax2(:,3),'g','LineWidth',10) %x-tal Y"
    plot3(Ax3(:,1),Ax3(:,2),Ax3(:,3),'r','LineWidth',10) %x-tal Z"
    plot3(Pnts(:,1),Pnts(:,2),Pnts(:,3),'.k','MarkerSize',50)
