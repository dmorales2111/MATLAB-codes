...
figure('units','pixels','position',[0.1*scrsz(3) 0.1*scrsz(4) 1.5*scrsz(3)/3 0.8*scrsz(4)],...
       'name','XTALFIT1_build',...  %'menubar','none',...
       'numbertitle','off','ToolBar','none',...
       'resize','off');

subplot('position',[0.05,0.1,0.5,0.75])
...
%slider1 = height
hui.txt(1) = uicontrol('style','text','string','fit height','position',[560 800 40 25]);
hui.ed(1) = uicontrol('style','edit',...
    'string',num2str(initvalues(1)),'position',[560 775 40 25]);
hui.sl(1)=uicontrol('style','slider',...
'position',[570 575 25 200],...
'sliderstep',[0.015 1],...
'value',initvalues(1),...%value has to be within min/max range
'min',minvalues(1),'max',maxvalues(1)); 

%slider2 = width
hui.txt(2) = uicontrol('style','text','string','fit width','position',[560 530 40 25]);
hui.ed(2) = uicontrol('style','edit',...
    'string',num2str(initvalues(2)),'position',[560 505 40 25]);
hui.sl(2)=uicontrol('style','slider',...
'position',[570 305 25 200],...
'sliderstep',[0.01 1],...
...
hui.txt(12) = uicontrol('style','text','string','offset','position',[560 260 40 25]);
hui.ed(12) = uicontrol('style','edit',...
    'string',num2str(initvalues(9)),'position',[560 235 40 25]);
hui.sl(12)=uicontrol('style','slider',...
'position',[570 35 25 200],...
'sliderstep',[0.01 1],...
'value',initvalues(9),...%value has to be within min/max range
'min',minvalues(9),'max',maxvalues(9)); 

%slider3 = Txx
hui.txt(3) = uicontrol('style','text','string','Txx','position',[670 800 40 25]); %,'fontsize',10);
hui.ed(3) = uicontrol('style','edit','string',num2str(0),'position',[670 775 40 25]); %,'fontsize',10);
hui.sl(3) = uicontrol('style','slider','position',[680 575 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(3),...%value has to be within min/max range
'min',minvalues(3),'max',maxvalues(3));

%slider4 = Tyy
hui.txt(4) = uicontrol('style','text','string','Tyy','position',[720 800 40 25]); %,'fontsize',10);
hui.ed(4) = uicontrol('style','edit','string',num2str(0),'position',[720 775 40 25]); %,'fontsize',10);
hui.sl(4) = uicontrol('style','slider','position',[725 575 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(4),...%value has to be within min/max range
'min',minvalues(4),'max',maxvalues(4));

%slider5 = Tzz
hui.txt(5) = uicontrol('style','text','string','Tzz','position',[765 800 40 25]); %,'fontsize',10);
hui.ed(5) = uicontrol('style','edit','string',num2str(0),'position',[765 775 40 25]); %,'fontsize',10);
hui.sl(5) = uicontrol('style','slider','position',[775 575 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(5),...%value has to be within min/max range
'min',minvalues(5),'max',maxvalues(5));

%slider6 = Txy
hui.txt(6) = uicontrol('style','text','string','Txy','position',[765 530 40 25]); %,'fontsize',10);
hui.ed(6) = uicontrol('style','edit','string',num2str(0),'position',[765 505 40 25]); %,'fontsize',10);
hui.sl(6) = uicontrol('style','slider','position',[775 305 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(6),...%value has to be within min/max range
'min',minvalues(6),'max',maxvalues(6));

%slider7 = Txz
hui.txt(7) = uicontrol('style','text','string','Txz','position',[720 530 40 25]); %,'fontsize',10);
hui.ed(7) = uicontrol('style','edit','string',num2str(0),'position',[720 505 40 25]); %,'fontsize',10);
hui.sl(7) = uicontrol('style','slider','position',[725 305 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(7),...%value has to be within min/max range
'min',minvalues(7),'max',maxvalues(7));

%slider8 = Tyz
hui.txt(8) = uicontrol('style','text','string','Tyz','position',[670 530 40 25]); %,'fontsize',10);
hui.ed(8) = uicontrol('style','edit','string',num2str(0),'position',[670 505 40 25]); %,'fontsize',10);
hui.sl(8) = uicontrol('style','slider','position',[680 305 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(8),...%value has to be within min/max range
'min',minvalues(8),'max',maxvalues(8));

%slider9 = alpha
hui.txt(9) = uicontrol('style','text','string','alpha','position',[670 260 40 25]); %,'fontsize',10);
hui.ed(9) = uicontrol('style','edit','string',num2str(0),'position',[670 235 40 25]); %,'fontsize',10);
hui.sl(9) = uicontrol('style','slider','position',[680 35 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(9),...%value has to be within min/max range
'min',minvalues(9),'max',maxvalues(9));

%slider10 = beta
hui.txt(10) = uicontrol('style','text','string','beta','position',[720 260 40 25]); %,'fontsize',10);
hui.ed(10) = uicontrol('style','edit','string',num2str(0),'position',[720 235 40 25]); %,'fontsize',10);
hui.sl(10) = uicontrol('style','slider','position',[725 35 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(9),...%value has to be within min/max range
'min',minvalues(9),'max',maxvalues(9));

%slider11 = gamma
hui.txt(11) = uicontrol('style','text','string','gamma','position',[765 260 40 25]); %'fontsize',10);
hui.ed(11) = uicontrol('style','edit','string',num2str(0),'position',[765 235 40 25]); %,'fontsize',10);
hui.sl(11) = uicontrol('style','slider','position',[775 35 25 200],'sliderstep',[0.005 0.1],...
'value',initvalues(9),...%value has to be within min/max range
'min',minvalues(9),'max',maxvalues(9));
...
R = Rot*Rg*Rb*Ra*Orient; %rotation matrix (oriented first - then tweeked by Euler angles - then rotated)

Minp=R*Min*(R\I0); %Input Tensor transformed to fit data
...
Fitx = updatefig_1(Tenval,Orienit,alf,bet,gam,ofst,h,w);
subplot('position',[0.05,0.1,0.5,0.75])
...
for i=2:dimtem(2)/2
    j=2*i-1;
    THET=theta(j)+ofst;
    tempx=updatefig_1(Tenval,Orienit,alf,bet,gam,THET,h,w); %call the main fitting function for each theta
    plot(tempx(:,1),tempx(:,2)+theta(j),'r',redat(:,j),redat(:,j+1),'.k')
end
hold off
xlabel('ppm') 
