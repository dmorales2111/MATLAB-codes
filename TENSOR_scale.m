...
hui.fh = figure('Units','pixels','Position',[0.05*scrsz(3) 0.05*scrsz(4) 0.85*scrsz(3) 0.80*scrsz(4)],...
    'Name','Tensor_scale',...  %'menubar','none',...
...
hui.txt(1) = uicontrol('style','text','string','G','units','normalized','position',...
...
hui.txt(2) = uicontrol('style','text','string','B','units','normalized',...
...
hui.txt(3) = uicontrol('style','text','string','A','units','normalized','position',...
...
Ra1=[cos(par(3)) sin(par(3)) 0;-sin(par(3)) cos(par(3)) 0;0 0 1]; 
Rb2=[cos(par(2)) 0 -sin(par(2));0 1 0;sin(par(2)) 0 cos(par(2))];
Rg3=[cos(par(1)) sin(par(1)) 0;-sin(par(1)) cos(par(1)) 0;0 0 1];
...
OUT_data.ten = Mdat; %lab-frame tensor
OUT_data.orn = Orien; %xtal-frame orientation relative to lab-frame
