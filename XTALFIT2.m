function [] = XTALFIT2(datax,datay,dataz)
...
%     xui3pos = get(ax3,'Position');
    ...
    hui.txt(9) = uicontrol('style','text','string','alpha','units','normalized','position',[(ax3l+ax3w+55)/fhwidth (ax3b+ax3h-ax3h*0.25-ax3h*0.05-45-ax3h*0.25-45-ax3h*0.05)/fhheight ax3w*0.15/fhwidth ax3h*0.05/fhheight]); %,'fontsize',10);
    ...
    hui.txt(10) = uicontrol('style','text','string','beta','units','normalized','position',[(ax3l+ax3w+80+ax3w*0.15)/fhwidth (ax3b+ax3h-ax3h*0.25-ax3h*0.05-45-ax3h*0.25-45-ax3h*0.05)/fhheight ax3w*0.15/fhwidth ax3h*0.05/fhheight]); %,'fontsize',10);
    ...
    hui.txt(11) = uicontrol('style','text','string','gamma','units','normalized','position',[(ax3l+ax3w+105+2*ax3w*0.15)/fhwidth (ax3b+ax3h-ax3h*0.25-ax3h*0.05-45-ax3h*0.25-45-ax3h*0.05)/fhheight ax3w*0.15/fhwidth ax3h*0.05/fhheight]); %'fontsize',10);
...
subplot('position',[(fhleft+25+fhwidth*0.2+55)/fhwidth (fhbottom+50)/fhheight fhwidth*0.2/fhwidth fhheight*0.70/fhheight])
...
subplot('position',[(fhleft+25+2*fhwidth*0.2+110)/fhwidth (fhbottom+50)/fhheight fhwidth*0.2/fhwidth fhheight*0.70/fhheight])
...
%***************************************************
function z = updatefig_2(Min,Orient,R2,af,bt,gm,offs,tht,h,w) %tht is the offset angle for the fixed tensor
...
%{
Rota = [1 0 0;0 cos(offs+tht) sin(offs+tht);0 -sin(offs+tht) cos(offs+tht)]; %rotations of THETA+offset for fixed tensor about lab x-axis
Rotb = [1 0 0;0 cos(tht) sin(tht);0 -sin(tht) cos(tht)]; %rotations of THETA about lab x-axis

Ra = Rota*R2;
Min1=Ra*Min*(Ra\I0); %1st tensor (fixed except for lab x-axis rotations)

Rb = Rotb*EULer_calc(af,bt,gm,Orient)*R2; %rotation matrix
Min2=Rb*Min*(Rb\I0); %2nd tensor (adujst with Euler angles)
%}

Roff = [1 0 0;0 cos(offs) sin(offs);0 -sin(offs) cos(offs)]; %rotations of THETA+offset for fixed tensor about lab x-axis
Rot = [1 0 0;0 cos(tht) sin(tht);0 -sin(tht) cos(tht)]; %rotations of THETA about lab x-axis

Rotc = Roff*R2*Orient; %Rotc = orientation / Euler / offset transformations
Ra = Rot*Rotc; %Ra transforms Rotc into lab rotations about x-axis
Min1=Ra*Min*(Ra\I0); %1st tensor (fixed except for lab x-axis rotations)

Rb = Rot*EULer_calc(af,bt,gm)*Rotc; %rotation transformation for 2nd tensor
Min2=Rb*Min*(Rb\I0); %2nd tensor (adujst with Euler angles)
...
R2x=EULer_calc(OUT_data.ang(1,1),OUT_data.ang(2,1),OUT_data.ang(3,1));
...
Fitx = updatefig_2(Tenval,Orienitx,R2x,alf,bet,gam,OUT_data.ang(4,1),0,h,w);
subplot('position',[(fhleft+25)/fhwidth (fhbottom+50)/fhheight (fhwidth*0.2)/fhwidth fhheight*0.70/fhheight])
...
for i=2:dimtem(2)/2
    j=2*i-1;
    THET=theta(j);
    tempx=updatefig_2(Tenval,Orienitx,R2x,alf,bet,gam,OUT_data.ang(4,1),THET,h,w); %call the main fitting function for each theta
    plot(tempx(:,1),tempx(:,2)+theta(j),'r',realdatax(:,j),realdatax(:,j+1),'.k')
end
hold off
xlabel('ppm') 
...
R2y=EULer_calc(OUT_data.ang(1,1),OUT_data.ang(2,1),OUT_data.ang(3,1));
...
Fity = updatefig_2(Tenval,Orienity,R2y,alf,bet,gam,OUT_data.ang(4,1),0,h,w);
subplot('position',[(fhleft+25+fhwidth*0.2+55)/fhwidth (fhbottom+50)/fhheight fhwidth*0.2/fhwidth fhheight*0.70/fhheight])
...
for i=2:dimtem(2)/2
    j=2*i-1;
    THET=theta(j);
    tempy=updatefig_2(Tenval,Orienity,R2y,alf,bet,gam,OUT_data.ang(4,1),THET,h,w); %call the main fitting function for each theta
    plot(tempy(:,1),tempy(:,2)+theta(j),'r',realdatay(:,j),realdatay(:,j+1),'.k')
end
hold off
xlabel('ppm') 
...
R2z=EULer_calc(OUT_data.ang(1,1),OUT_data.ang(2,1),OUT_data.ang(3,1));
...
Fitz = updatefig_2(Tenval,Orienitz,R2z,alf,bet,gam,OUT_data.ang(4,1),0,h,w);
subplot('position',[(fhleft+25+2*fhwidth*0.2+110)/fhwidth (fhbottom+50)/fhheight fhwidth*0.2/fhwidth fhheight*0.70/fhheight])
...
for i=2:dimtem(2)/2
    j=2*i-1;
    THET=theta(j);
    tempz = updatefig_2(Tenval,Orienitz,R2z,alf,bet,gam,OUT_data.ang(4,1),THET,h,w);
    plot(tempz(:,1),tempz(:,2)+theta(j),'r',realdataz(:,j),realdataz(:,j+1),'.k')
end
hold off
xlabel('ppm') 
